package com.gordan.framework.db;
...
public abstract class SqliteHelper<T>
{
	 ...
	 */
	protected long insert(Context mContext,String table, ContentValues values)
	{
		long num = 0l;
		try
		{
	 ...
	 * @return
	 */
	protected void insert(Context mContext,String sql)
	{
		try
		{
			open(mContext);
	 ...
	 * @param sql
	 */
	protected void delete(Context mContext,String sql)
	{
		try
		{
			open(mContext);
	 ...
	 * @return 返回删除的条数
	 */
	protected int delete(Context mContext,String table, String whereClause, String[] whereArgs)
	{
		int num = 0;
		try
		{
	 ...
	 * @return 返回修改的条数
	 */
	protected int update(Context mContext,String table, ContentValues values, String whereClause, String[] whereArgs)
	{
		int num = 0;
		try
		{
	...
	protected void update(Context mContext,String sql)
	{
		try
		{
			open(mContext);
	...
	public T queryObject(Context mContext,Class<?> classObj, String table, String[] columns, String selection, String[] selectionArgs)
	{
		Cursor c = null;
		T t = null;
		try
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected List<T> queryBySql(Context mContext,Class<?> classObj, String sql)
	{
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected List<T> queryAllList(Context mContext,Class<?> classObj, String table, String[] columns, String selection, String[] selectionArgs)
	{
		Cursor c = null;
		T t = null;
		List<T> list = null;
	 ...
	 * 打开连接
	 */
	private void open(Context mContext)
	{
		try
		{
			if(mDbHelper==null)
